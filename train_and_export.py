...
train_and_export.py
====================
Trains all 4 models on IMDB dataset and exports every result to JSON.
NO hardcoded / synthetic data — everything comes from actual training.

Usage:
    pip install -r backend/requirements.txt
    python train_and_export.py

Optional (for LSTM + DistilBERT):
    pip install tensorflow transformers datasets

Output: backend/data/exports/*.json
"""
...
# Ensure Transformers uses tf-keras (legacy) for TensorFlow 2.16+ compatibility
os.environ["TF_USE_LEGACY_KERAS"] = "1"

# ── Multiprocess Fix for Python 3.12 ──────────────────────────────────────────
# Hack to fix "AttributeError: '_thread.RLock' object has no attribute '_recursion_count'"
# which happens in multiprocess 0.70.18 during shutdown on Python 3.12.
try:
    ...
    def patched_stop_locked(self, *args, **kwargs):
        # The error happens at: if self._lock._recursion_count() > 1:
        # We safely bypass it if the attribute is missing.
        if hasattr(self._lock, "_recursion_count"):
            return orig_stop_locked(self, *args, **kwargs)
        
        # If _recursion_count is missing, we just proceed to the rest of the cleanup
        # manually or by skipping the reentrant check.
...
try:
    ...
    try:
        ...
        print("Please either:")
        print("  1) pip install datasets   (auto-download)")
        print("  2) Place 'IMDB Dataset.csv' in the project root")
        raise SystemExit(1)
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODEL 3 — Bidirectional LSTM  (requires tensorflow)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# MODEL 4 — DistilBERT  (requires transformers + tensorflow)
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*60 + "\nTraining DistilBERT...\n" + "="*60)
...
try:
    import tensorflow as tf
    # Ensure tf-keras is used if available
    try:
        import tf_keras
    except ImportError:
        pass
    ...
    bert_hist_obj = bert_model.fit(train_ds, validation_data=val_ds,
                                    epochs=BERT_EPOCHS, verbose=1)
    ...
    print("  Install: pip install transformers tensorflow")

# ═══════════════════════════════════════════════════════════════════════════════
# LIME Explanations  (requires lime)
# ═══════════════════════════════════════════════════════════════════════════════
...
try:
    ...
    easy_pos = [i for i,(yt,yp,ypr) in enumerate(zip(y_test,nb_pred,nb_proba))
                if yt==1 and yp==1 and ypr>0.92][:4]
    easy_neg = [i for i,(yt,yp,ypr) in enumerate(zip(y_test,nb_pred,nb_proba))
                if yt==0 and yp==0 and ypr<0.08][:4]
    ...
    for model_name, predict_fn in [
        ...
        for idx in sample_indices:
            try:
                exp        = explainer.explain_instance(X_test_raw[idx], predict_fn,
                ...
                lime_results.append({
                    "id": len(lime_results), "model": model_name,
                    "text": X_test_raw[idx][:500],
                    "true_label": true_label, "pred_label": pred_label,
                    "correct": true_label==pred_label, "confidence": round(conf,4),
                    "words": [{"word": w, "weight": round(float(wt),4)} for w,wt in exp.as_list()],
                })
...
length_dist = [{"bucket": f"{int(edges[i])}-{int(edges[i+1])}",
                "positive": int(pos_hist[i]), "negative": int(neg_hist[i])}
...
for lo, hi in [(0,300),(300,600),(600,1000),(1000,1500),(1500,99999)]:
    ...
    if yt_b:
        len_acc_data.append({"bucket": f"{lo}-{hi}" if hi<99999 else f"{lo}+",
                              "count": len(yt_b),
                              "accuracy": round(float(accuracy_score(yt_b,yp_b)),4)})
...
for i in range(min(100, len(test_df))):
    r = test_df.iloc[i]
    sample_reviews.append({"id": i+1, "text": str(r["review"])[:300],
                            "sentiment": int(r["label"]), "length": len(str(r["review"]))})

dataset_stats = {
    "total_reviews": len(df), "train_size": len(train_df), "test_size": len(test_df),
    "length_distribution": length_dist,
    "length_stats": {
        "overall":  {"mean": round(float(df["length"].mean()),1),
                     "median": round(float(df["length"].median()),1),
                     "std": round(float(df["length"].std()),1)},
        "positive": {"mean": round(float(pos_df["length"].mean()),1),
                     "median": round(float(pos_df["length"].median()),1)},
        "negative": {"mean": round(float(neg_df["length"].mean()),1),
                     "median": round(float(neg_df["length"].median()),1)},
    },
    ...
    "length_vs_accuracy":  len_acc_data,
...
for m1 in model_names:
    ...
    for m2 in model_names:
        p1 = np.array(all_model_preds[m1])
        p2 = np.array(all_model_preds[m2])
        n  = min(len(p1), len(p2))
...
for i in range(min_len):
    ...
    if all(int(p) != int(y_test[i]) for p in preds):
        all_wrong.append({"idx": i, "review": X_test_raw[i][:300],
...
training_history = {}
if lstm_history:
    training_history.update(lstm_history)
if bert_history:
    training_history.update(bert_history)

print("\n" + "="*60 + "\nSaving exports...\n" + "="*60)

exports = {
    "metrics.json":            all_metrics,
    "confusion_matrices.json": all_cm,
    "roc_curves.json":         all_roc,
    "pr_curves.json":          all_pr_curves,
    "training_history.json":   training_history,
    "error_samples.json":      all_errors,
    "lime_examples.json":      lime_results,
    "confidence_dist.json":    all_conf_dist,
    "feature_importance.json": all_feature_imp,
    "dataset_stats.json":      dataset_stats,
    "model_agreement.json":    {"matrix": agreement, "all_wrong": all_wrong},
}

for fname, data in exports.items():
    path = EXPORT_DIR / fname
    path.write_text(json.dumps(data, indent=2))
    print(f"  OK  {fname:38s} ({path.stat().st_size/1024:.1f} KB)")
...
for name, m in all_metrics.items():
    print(f"  {name:25s}  Acc={m['Accuracy']:.4f}  F1={m['F1-Score']:.4f}  AUC={m['AUC-ROC']:.4f}")
